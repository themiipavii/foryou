(function($) {
	...
	if (typeof Ephesus!=="undefined") {
		$.extend(EphesusOptions, Ephesus);
	}
	...
	EphesusOptions.mobile = $.browser.mobile;
	...
	$.EphesusTheme = {
		...
		background:function() {
			...
			//Lines
			if ($(".bg.linesbg").length>0) {
				$(".bg.linesbg").vegas({
					slides:[
						{src:"images/bg/lines.jpg"},
					],
					transition:"fade"
				});		
			}
		...
		sidebar:function() {
			...
			$('.info-nav, .more-info, .nav').on("click", function(e) {
				...
				// Button
				if (btn.hasClass("nav") || btn.hasClass("info-nav")) {
					if (btn.attr("href")!=="#") {
						$("#about").modal("show");
			...
			$("#about").on("shown.bs.modal", function() {
				if (!EphesusOptions.maps_created) {
					setTimeout(function() {
						that.map();
						EphesusOptions.maps_created = true;
					}, 500);
				}
			});
		},
		...
		map:function(color, marker) {
			...
			if (EphesusOptions.zoom_div===null) {		
				var zoomControlDiv = document.createElement("div");		
				var zoomControl = new customZoomControl(zoomControlDiv, map);
				EphesusOptions.zoom_div = zoomControlDiv;
			}

			//Insert the zoom div on the top left of the map
			map.controls[google.maps.ControlPosition.LEFT_TOP].push(EphesusOptions.zoom_div);
		}
